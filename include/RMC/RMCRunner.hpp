...
// Reusable simulation setup: turns inputs into a ready Engine via build_engine.
...
// Assemble an engine from already-loaded inputs (copies the structure). Vary
// cfg.seed per replica to build N replicas from one disk read.
[[nodiscard]] Engine build_engine(const LoadedStructure &loaded,
                                  const ExperimentalData &data,
                                  const RMCConfig &cfg);

// Load structure + experimental data from disk, then build_engine().
[[nodiscard]] Result<Engine> build_engine(const RMCConfig &cfg);

// Apply cfg.move_gen to an already-built engine in its FINAL location (gradient
// generators bind a pointer to engine.constraints(), invalidated by moving the
// engine). Random is a no-op; Langevin/Leapfrog rebuild with the gradient proposer.
...
// (.vasp/.poscar → VASP, .lammps/.lmp/.data → LAMMPS, else PDB). `box` is the
// active periodic cell (zero for infinite).

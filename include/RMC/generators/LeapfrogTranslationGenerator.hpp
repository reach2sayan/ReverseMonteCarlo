...
//
// rejection_override() satisfies CMoveGeneratorWithRejectionOverride — the
// Engine's settle() uses this instead of constraints_.should_reject().
struct LeapfrogTranslationGenerator
    ...
    void generate(IMoveGenerator::Token, coords_t &coords,
        ...
        Eigen::MatrixXd saved(k, 3);
        for (Eigen::Index ai = 0; ai < k; ++ai)
            saved.row(ai) = coords.row(static_cast<Eigen::Index>(
                indices[static_cast<std::size_t>(ai)]));

        // Initial half-step for momentum
        vec_t g = GradientOracle::translation_gradient(coords, indices, *constraints);
        p -= (step_size / 2.0) * g;
        ...
        for (int t = 0; t < n_steps; ++t) {
            // Position full-step
            for (Eigen::Index ai = 0; ai < k; ++ai) {
                const auto atom = static_cast<Eigen::Index>(
                    indices[static_cast<std::size_t>(ai)]);
                for (int ax = 0; ax < 3; ++ax)
                    coords(atom, ax) += step_size * p[3 * ai + ax];
            }
            ...
            if (nuts_mode) {
                ...
                for (Eigen::Index ai = 0; ai < k; ++ai) {
                    const auto atom = static_cast<Eigen::Index>(
                        indices[static_cast<std::size_t>(ai)]);
                    r_diff_flat[3 * ai + 0] = coords(atom, 0) - saved(ai, 0);
                    r_diff_flat[3 * ai + 1] = coords(atom, 1) - saved(ai, 1);
                    r_diff_flat[3 * ai + 2] = coords(atom, 2) - saved(ai, 2);
                }
        ...
        if (rejected) {
            ...
            for (Eigen::Index ai = 0; ai < k; ++ai)
                coords.row(static_cast<Eigen::Index>(
                    indices[static_cast<std::size_t>(ai)])) = saved.row(ai);
        }

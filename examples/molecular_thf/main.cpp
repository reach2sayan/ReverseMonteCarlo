...
static void add_thf(AtomicStructure &s, int mol_id,
...
int main() {
    ...
    for (int m = 0; m < n_mol; ++m) {
        ...
        Group gt; gt.name = "mol_t" + std::to_string(m); gt.indices = idx;
        gt.generator.emplace(TranslationGenerator(0.0, 0.2, static_cast<std::uint32_t>(m + 200)));
        eng.add_group(std::move(gt));

        Group gr; gr.name = "mol_r" + std::to_string(m); gr.indices = idx;
        gr.generator.emplace(RotationGenerator(0.0, 0.05, static_cast<std::uint32_t>(m + 300)));
    ...
    std::cout << "THF (" << n_mol << " mol): accepted " << eng.stats().steps_accepted

...
// Demonstrates random, axis-aligned, and combined translations on a single atom.
// No constraints: every proposed move is accepted.
...
int main() {
    ...
    struct Case { const char *label; IMoveGenerator gen; };
    std::vector<Case> cases;
    cases.push_back({"random direction",
        IMoveGenerator{TranslationGenerator(0.0, 0.3, 1)}});
    cases.push_back({"along X axis",
        IMoveGenerator{TranslationAlongAxisGenerator({1,0,0}, 0.0, 0.3, 2)}});
    cases.push_back({"along Y axis",
        IMoveGenerator{TranslationAlongAxisGenerator({0,1,0}, 0.0, 0.3, 3)}});
    cases.push_back({"along Z axis",
        IMoveGenerator{TranslationAlongAxisGenerator({0,0,1}, 0.0, 0.3, 4)}});

...
static void run_with(const std::string &label, IGroupSelector selector) {
    ...
    for (std::size_t i = 0; i < 10; ++i) {
        ...
        g.indices = {i};
        g.generator.emplace(TranslationGenerator(0.0, 0.3, static_cast<std::uint32_t>(i + 1)));
    ...
    double accept_rate = eng.stats().steps_tried > 0
        ? static_cast<double>(eng.stats().steps_accepted) / static_cast<double>(eng.stats().steps_tried)
        : 0.0;

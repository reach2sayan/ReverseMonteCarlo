...
AtomicStructure make_structure(
    std::initializer_list<std::array<double, 3>> positions,
...
// ─── 1. translations ──────────────────────────────────────────────────────────
// fullrmc example: whole-molecule random translation, no constraints.
...
// ─── 2. rotations ─────────────────────────────────────────────────────────────
// fullrmc example: whole-molecule rotation — all pairwise distances preserved.
          ...
          "[example_rotations]")
{
    ...
    const auto dist_ij = [&](int a, int b) {
        return (engine.structure().coordinates.row(a) -
                engine.structure().coordinates.row(b)).norm();
    };
...
// ─── 3. agitations ────────────────────────────────────────────────────────────
// fullrmc example: bond and angle constraints on a water-like molecule with
// per-atom move groups.  Initial geometry already satisfies both; constraints
// prevent any worsening move.
TEST_CASE("agitations - bond and angle constraints maintained on water geometry",
          "[example_agitations]")
{
...
// ─── 4. bondsConstraint ───────────────────────────────────────────────────────
// fullrmc example: BondConstraint holds structure near target bond lengths.
// Scenario A: normal target — bonds stay in range from the start.
// Scenario B: distorted target — engine moves atoms toward the new target.
...
TEST_CASE("bondsConstraint - distorted bond target produces accepted moves",
          "[example_bonds_constraint]")
{
    // Initial bonds 1.0 Å; target [2.2,2.5] and [0.2,0.5] — engine must move atoms.
    auto s = make_structure(
        {{0,0,0},{1,0,0},{0,1,0}},
        {"O","H","H"},
        {0,0,0});
...
// ─── 5. anglesConstraint ──────────────────────────────────────────────────────
// fullrmc example: AngleConstraint steers the H-O-H angle.
...
// fullrmc example: DihedralAngleConstraint drives a 4-atom chain between
// rotamer shells (like butane C1-C2-C3-C4).
TEST_CASE("dihedralConstraint - permissive dihedral completes without rejection",
          "[example_dihedral_constraint]")
{
    // Non-collinear chain so the dihedral is well-defined.
    auto s = make_structure(
        {{0,0,0},{1,0,0},{2,0,1},{3,1,1}},
        {"C","C","C","C"},
        {0,0,0,0});
...
// fullrmc example: ImproperAngleConstraint enforces planarity of a molecular
// fragment (XeF5-like).
TEST_CASE("improperConstraint - tight planarity constraint holds near-planar geometry",
...
TEST_CASE("improperConstraint - out-of-plane start drives toward planarity",
          "[example_improper_constraint]")
{
    // 4th atom strongly out of plane → large initial improper; engine accepts moves
    // that push the fragment back toward planarity.
    auto s = make_structure(
        {{0,0,0},{1,0,0},{0.5,0.866,0},{0.5,0.289,1.0}},
        {"Xe","F","F","F"},
        {0,0,0,0});
...
// fullrmc example: AtomicCoordinationNumberConstraint locks the coordination
// number of a central atom (Al) to exactly 2 Cl neighbours in a shell.
TEST_CASE("coordNumConstraint - coordination number held at 2 after 2000 steps",
          "[example_coord_num_constraint]")
{
    // Al(0) at origin; Cl(1,2) at ±2 Å → CN = 2 (in [1.5,2.5]); Cl(3,4) at ±10 Å → outside.
    auto s = make_structure(
    ...
    for (std::size_t idx : {1u, 2u, 3u, 4u}) {
        ...
        g.indices = {idx};
        g.generator.emplace(TranslationGenerator(0.0, 0.1, 50u + static_cast<std::uint32_t>(idx)));
        engine.add_group(std::move(g));
    }
...
// ─── 9. removes ───────────────────────────────────────────────────────────────
// fullrmc example: AtomsRemoveGenerator stages atomic removals that the engine
// either commits (accepted) or rolls back (rejected).

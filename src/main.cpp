...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()("help,h", "Show this help")(
      ...
      "MC steps")("rho0", po::value<double>()->default_value(0.1),
  ...
  return leaf::try_handle_all(
      [&]() -> leaf::result<int> {
        ...
        // ---- Engine ----
        RMC::Engine engine(std::move(s), std::move(bc));
        engine.build_atomic_groups(0.0, 0.2, vm["seed"].as<std::uint32_t>());

        if (vm["smart"].as<bool>())
          engine.set_selector(
              RMC::SmartRandomSelector{1.1, vm["seed"].as<std::uint32_t>()});
        ...
        if (vm.count("sq")) {
          BOOST_LEAF_AUTO(data,
                          RMC::io::read_xy_data(vm["sq"].as<std::string>()));
        ...
        engine.set_step_callback(
            [](std::uint64_t step, std::uint64_t acc, std::uint64_t tried,
              ...
              BOOST_LOG_TRIVIAL(info)
                  ...
                  << std::setprecision(1) << rate << "%" << "  chi2=" << chi2;
            },
            1000);

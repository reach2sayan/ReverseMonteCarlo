...
void StructureFactorConstraint::set_experimental_data(const mat_t& data) {
    if (data.cols() < 2)
        throw std::invalid_argument("StructureFactorConstraint: need 2-column Q/S(Q) data");
    exp_Q_  = data.col(0);
    exp_S_  = data.col(1);
}
...
void StructureFactorConstraint::initialise() {
    ...
    if (elements_) pdf_->set_elements(elements_);
    pdf_->initialise();
    ...
    for (Eigen::Index qi = 0; qi < nQ; ++qi) {
        ...
        for (int ri = 0; ri < n_r_bins_; ++ri) {
            real_t r = r_min_ + (ri + 0.5) * r_bin_;
            Gr2Sq_(qi, ri) = (q > 1e-10)
                ? r_bin_ * std::sin(q * r) / q
                : 0.0;
        }
    }
...
real_t StructureFactorConstraint::compute_error(
    ...
    std::span<const index_t> moved) const
{
    ...
    real_t denom = computed_S_.squaredNorm();
    real_t scale = (denom > 1e-30) ? computed_S_.dot(exp_S_) / denom : 1.0;

...
const std::unordered_map<std::string, int> ATOMIC_NUMBERS = {
    ...
    {"Ar",18},{"K",19},{"Ca",20},{"Fe",26},{"Ni",28},{"Cu",29},{"Zn",30},
    {"Br",35},{"I",53},{"Pb",82}
};
        ...
        try {
            ...
            if (element.empty() && !atom_name.empty()) {
                ...
                for (char c : atom_name)
                    if (std::isalpha(c)) { element = std::string(1, c); break; }
            }
            ...
            s.atomic_numbers.conservativeResize(
                s.atomic_numbers.size() + 1);
...
Result<void> write_pdb(const AtomicStructure& s,
    ...
    std::ofstream f(path);
    if (!f) return std::unexpected("Cannot write PDB: " + path.string());

    for (Eigen::Index i = 0; i < s.coordinates.rows(); ++i) {
        const std::string& name = (i < static_cast<Eigen::Index>(s.names.size()))
                                    ? s.names[static_cast<std::size_t>(i)] : "X";
        const std::string& res  = (i < static_cast<Eigen::Index>(s.residues.size()))
                                    ? s.residues[static_cast<std::size_t>(i)] : "UNK";
        const std::string& elem = (i < static_cast<Eigen::Index>(s.elements.size()))
                                    ? s.elements[static_cast<std::size_t>(i)] : "";
        index_t mol = (i < static_cast<Eigen::Index>(s.molecule_ids.size()))
                        ? s.molecule_ids[static_cast<std::size_t>(i)] : 1;

        char buf[81];
        std::snprintf(buf, sizeof(buf),
            "ATOM  %5lld %-4s %-3s A%4d    %8.3f%8.3f%8.3f  1.00  0.00          %2s\n",
            static_cast<long long>(i + 1), name.c_str(), res.c_str(),
            static_cast<int>(mol),
            s.coordinates(i, 0), s.coordinates(i, 1), s.coordinates(i, 2),
            elem.c_str());
        f << buf;
    }
    f << "END\n";

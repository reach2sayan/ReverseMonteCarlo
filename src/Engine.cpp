...
void Engine::add_group(Group g) {
    groups_.push_back(std::move(g));
}

void Engine::build_atomic_groups(real_t min_amp, real_t max_amp,
                                   std::uint32_t seed) {
    groups_.clear();
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        g.indices = {static_cast<index_t>(i)};
...
io::EngineStats Engine::stats() const noexcept {
    return {n_steps_total_, n_steps_accepted_,
            n_steps_tried_, constraints_.total_error()};
}
...
void Engine::run_until(real_t target_chi2, std::uint64_t max_steps) {
    if (groups_.empty())
    ...
    while (constraints_.total_error() > target_chi2) {
        step();
        ++s;
        if (max_steps > 0 && s >= max_steps) break;
    }
}
...
void Engine::step() {
    ...
    if (step_cb_ && (n_steps_total_ % log_every_ == 0))
        step_cb_(n_steps_total_, n_steps_accepted_,
                  n_steps_tried_, constraints_.total_error());
...
void Engine::apply_pbc(std::span<const index_t> moved) {
    for (auto i : moved) {
